...
public class HTTPReqProcessor implements HTTPFileGetter
{
	...
	/**
	 * Runs a cgi script descripted by the given request path, for the given request object.
	 * Returns the response databauffer, parsed to oblivion into the given headers.
	 * @param reqPath the request url to parse
	 * @param request the request object to process
	 * @return the body of the data generated by the cgi script
	 * @throws HTTPException an exception
	 */
	protected DataBuffers executeCGI(final String reqPath, final HTTPRequest request, final Map<HTTPHeader, String> headers) throws HTTPException
	{
		int nextSlash=reqPath.indexOf('/',9);
		final String cgiPath=reqPath.substring(0,nextSlash<0?reqPath.length():nextSlash);
		final String cgiPathInfo = (nextSlash<0) ? "" : reqPath.substring(nextSlash);
	...
	public DataBuffers getFileData(HTTPRequest request) throws HTTPException
	{
		...
		if(reqPath.startsWith("/cgi-bin/") && (reqPath.length()>9))
			return executeCGI(reqPath,request,null);
	...
	public DataBuffers generateOutput(HTTPRequest request) throws HTTPException
	{
		...
		try
		{
			...
			if(buffers == null)
			{
				...
				if(reqPath.startsWith("/cgi-bin/") && (reqPath.length()>9))
				{
					buffers = executeCGI(reqPath,request,extraHeaders);
				}
				else
				{
					final File pageFile;
