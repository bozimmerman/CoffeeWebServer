...
public class ServletResponse implements SimpleServletResponse
{
	...
	{
		headers.put(CWHTTPHeader.CONTENT_TYPE.toString(), mimeType);
	}
	...
	public DataBuffers generateOutput(HTTPRequest request) throws HTTPException
	{
		...
		if(bout.size()>0)
		{
			if(!normalizedHeaders.contains(CWHTTPHeader.CONTENT_TYPE.lowerCaseName()))
				str.append(CWHTTPHeader.CONTENT_TYPE.makeLine(MIMEType.html.getType()));
			if(!normalizedHeaders.contains(CWHTTPHeader.CONTENT_LENGTH.lowerCaseName()))
				str.append(CWHTTPHeader.CONTENT_LENGTH.makeLine(bout.size()));
		}
		...
		if(!normalizedHeaders.contains(CWHTTPHeader.SERVER.lowerCaseName()))
			str.append(HTTPIOHandler.SERVER_HEADER);
		if(!normalizedHeaders.contains(CWHTTPHeader.CONNECTION.lowerCaseName()))
			str.append(HTTPIOHandler.CONN_HEADER);
		if(!normalizedHeaders.contains(CWHTTPHeader.KEEP_ALIVE.lowerCaseName()))
			str.append(CWHTTPHeader.getKeepAliveHeader());
		if(!normalizedHeaders.contains(CWHTTPHeader.DATE.lowerCaseName()))
			str.append(CWHTTPHeader.DATE.makeLine(HTTPIOHandler.DATE_FORMAT.format(new Date(System.currentTimeMillis()))));
		for(String key : cookies.keySet())
			str.append(CWHTTPHeader.SET_COOKIE.makeLine(key+"="+cookies.get(key)));
		str.append(EOLN);
